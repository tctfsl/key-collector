...
/**
 * Monkey is one of the child class to the abstract Piece class.
 *
...
public class Monkey extends Piece {
  ...
  public Monkey(String name, String path, int x, int y) {
    super(name, path, x, y);
  }
  ...
  public Vector<Vector<Integer>> calculateMoves(Cell[][] bttn, int playerRow, int playerColumn, ArrayList<Piece> keysHeld) {
    ...
    for(int i = (playerRow - 3); i < (playerRow + 4); i++) {
      for(int j = (playerColumn - 3); j < (playerColumn + 4); j++) {
        if (i >= 0 && j >= 0 && i <= 8 && j <= 8 && ((i == playerRow || j == playerColumn) || (Math.abs(i - playerRow) == Math.abs(j - playerColumn)))) {
